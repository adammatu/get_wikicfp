...
{
    public class GroupMemberAMemberB : GroupBase
    {
        public GroupMemberAMemberB()
        {
            Name = "A+B committee members then A+B committee members again";
        }
            ...
            foreach (int id in eventLinksGrouped.Keys)
            {
                ...
                while (id1 < items.Count)
                {
                    ...
                    while (id2 < items.Count)
                    {
                        ...
                        if (!personLinksGroupedFirst.ContainsKey(pid1))
                        {
                            personLinksGroupedFirst.Add(pid1, new Dictionary<int, GropuResultFlat>());
                        }
                        if (personLinksGroupedFirst[pid1].ContainsKey(pid2))
                        {
                            GropuResultFlat link = personLinksGroupedFirst[pid1][pid2];
                            double score = items[0].Score;
                            DateTime created = items[0].Created;
                            ...
                            //
                            if (!personLinksGroupedConsecutive.ContainsKey(pid1))
                            {
                                personLinksGroupedConsecutive.Add(pid1, new Dictionary<int, GropuResultFlat>());
                            }
                            if (personLinksGroupedConsecutive[pid1].ContainsKey(pid2))
                            {
                                GropuResultFlat link2 = personLinksGroupedConsecutive[pid1][pid2];
                                if (link2.Created > created)
                                {
                                    link2.Created = created;
                                    link2.Score = score;
                                }
                            }
                            else
                            {
                                personLinksGroupedConsecutive[pid1].Add(pid2, new GropuResultFlat()
                                {
                                    ID1 = pid1,
                                    ID2 = pid2,
                                    Created = created,
                        ...
                        {
                            personLinksGroupedFirst[pid1].Add(pid2, new GropuResultFlat()
                            {
                                ID1 = pid1,
                                ID2 = pid2,
